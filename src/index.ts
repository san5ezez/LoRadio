...
export default { async fetch(request: Request, env: Env): Promise<Response> { const url = new URL(request.url); if (url.pathname.startsWith('/api/')) { if (request.method === 'OPTIONS') return cors(new Response(null, { status: 204 })); const id = env.RADIO_STATE.idFromName('main'); const stub = env.RADIO_STATE.get(id); if (url.pathname === '/api/state') return stub.fetch(new Request(new URL('/state', url), request)); if (url.pathname === '/api/tracks') return cors(new Response(JSON.stringify(seedTracks), { headers: { 'Content-Type': 'application/json' } })); if (url.pathname === '/api/stream-socket') return stub.fetch(new Request(new URL('/socket', url), request)); if (url.pathname === '/api/vote') return stub.fetch(new Request(new URL('/vote', url), request)); if (url.pathname.startsWith('/api/admin/')) { if (url.pathname === '/api/admin/login') return login(request, env); if (!validAdmin(request, env)) return cors(Response.json({ error: 'Unauthorized' }, { status: 401 })); return stub.fetch(new Request(new URL(url.pathname.replace('/api', ''), url), request)); } } return env.ASSETS.fetch(request); } };
function cors(response: Response) { const headers = new Headers(response.headers); headers.set('Access-Control-Allow-Origin', '*'); headers.set('Access-Control-Allow-Methods', 'GET, POST, OPTIONS'); headers.set('Access-Control-Allow-Headers', 'Content-Type'); return new Response(response.body, { status: response.status, headers }); }
